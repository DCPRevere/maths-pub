...
Every theorem in this file is claimed to rest on Lean's three standard axioms
`propext, Classical.choice, Quot.sound` -- no `sorry`, no `native_decide`.  A
successful build does NOT establish that: a declaration whose source is free of
`sorry` can still depend on `sorryAx` through an import.  The audit has to be
run.  Elaborating this file prints the dependencies of all seven theorems; the
stored output is `.verification/certificate-axioms.txt` at the repository root.
-/

section AxiomAudit

#print axioms nonneg_of_certificate
#print axioms sigma0_eq_zero_of_certificate
#print axioms eq_of_certificate
#print axioms quadForm_nonneg
#print axioms quadForm_eq_zero_iff
#print axioms vanish_of_posDef_quadForm
#print axioms certificate_bound_and_uniqueness
