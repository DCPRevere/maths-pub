...
#align(center)[
  ...
  #v(0.25em)
  #text(8.5pt, style: "italic")[Draft — 31 July 2026]
  #v(0.9em)
...
#block(inset: (x: 1.0cm))[
  #text(9pt)[
    *Abstract.* Cheon and Hwang conjectured in 1992 that
    ...
    factor two — is kernel-checked at every $k$ it states, $2 lt.eq k lt.eq 5$,
    over the whole range it states for each, and is of independent interest.
    Fixed-dimension certificates settle $(k=4, 5 lt.eq n lt.eq 9)$, so
    the line $k = 4$ is settled for every $n gt.eq 5$. Every result
...
#keybox[
  *Theorem G (stability of the Tverberg–Friedland minimum).* Let
  ...
  #leanline[_Kernel-checked at every stated cell_: $(k=2$, every
  $n gt.eq 2)$ and $(k=3$, every $n gt.eq 4)$ — `stabilityAt_two`,
  `stabilityAt_three` in `StabilityK3.lean` at commit `1507013`; $(k=4$,
  every $n gt.eq 8)$ — `stabilityAt_four` in `StabilityK4.lean` at commit
  `944b517`; and $(k=5$, every $n gt.eq 14)$ — `stabilityAt_five` in
  `StabilityK5.lean` at commit `45d20c2` — in each case over the whole range
  the theorem states for that $k$. @sec-machine states the limits.]
]
...
$(k = 4, 5 lt.eq n lt.eq 9)$. All five are settled by fixed-dimension
certificates at anchor grade, so the line $k = 4$ is settled for every
$n gt.eq 5$. @sec-cells records the five cells and what each rests on.
...
#figure(
  table(
    ...
    [$(k=4, n=9)$], [settled: $Phi_4 lt.eq 4366\/2187$ on $K_9$],
      [anchor grade on a stored exact witness: identity over all 1,355,805
       coefficients, equality only at $J_9\/9$, conjugacy exact, assembled
       positivity (cone size 3402) by the congruence route below],
    table.hline(stroke: 0.7pt + luma(40)),
  ),
  caption: [The five cells between Theorem A's line and Theorem H's range, as
    of 31 July 2026. All five are settled, and in each equality holds only at
    $J_n\/n$.]
) <k4-anchor-table>

At $(k=4, n = 7, 8, 9)$ the assembled Grams are positive definite over $QQ$
by _congruence_ rather than by a single direct factorisation. The 21
components — each the span of the group translates of one canonical block
basis — have translates that span, and distinct components are exactly
$H$-orthogonal over $QQ$; so the matrix $W$ of translates is invertible over
$QQ$ and $W^T H W$ is block diagonal, and by Sylvester's law the assembled
Gram is definite exactly when every component block is. At $n = 7$ and
$n = 8$ the component blocks were shown definite through their Kronecker
structure $C_b kron h_b$, all 21 of each factor definite under exact
$L D L^T$; at $n = 9$ they are factorised directly, which is a shorter chain
and costs $sum_b (dim S_b)^3$ rather than $B^3$. The conjugacy then extends
the verdict to all multiplier Grams (49 at $n = 7$, 64 at $n = 8$, 81 at
$n = 9$). Every link is a stored artefact, verified exactly over $QQ$ with
rejection controls that fire.

Two remarks on that route. First, orthogonality is not a property of the
stored witness: $H$ is invariant under the acting group for _any_ orbit
coefficients, so by Schur it is enough that the components share no
irreducible constituent — a witness-free condition, and one that is itself
an exact finite check, since the pair-orbit indicators span the symmetric
invariant matrices. That check is carried out at $n = 5, 6, 7, 8, 9$.
Second, at $(k=4, n=7)$ the verdict also has a second, independent proof:
both assembled $1274 times 1274$ Grams were factorised directly over $QQ$,
least pivots $1.637015 times 10^(-5)$ and $3.365027 times 10^(-6)$, in exact
agreement with the congruence route. The two component computations there use
different bases of the same components, and the check reconciling them is a
_consistency check between stored artefacts, not an independent
verification_.

_What this does and does not give._ Unconditionally, the line $k = 4$ is
settled for $5 lt.eq n lt.eq 9$ at anchor grade and for every $n gt.eq 10$ by
Theorem H, hence for every $n gt.eq 5$. What closes it is five exact
certificates and one theorem, not a single argument: the five cells rest on
stored rational data and an independent standalone verifier, at
exact-verifier grade and not in Lean, and no theorem of this paper depends on
any of them. A certificate family in $QQ(n)$ at $k = 4$ would still be worth
having — it would need no anchors, would supersede the local route, and would
inherit the $k = 3$ formalisation pattern. The groundwork for it is a decided
sweep rather than a certificate, recorded in the kit with its witnesses; two
of its facts bear on the method's reach. Mixed Gram degrees are ruled out for even $k$, since they
...
`944b517`, and — closing the last stated cell — `StabilityK5Sieve.lean`
(67), `StabilityK5.lean` (19) and `UniformStability.lean` (16) at `45d20c2`,
which subsume the formerly partial `StabilityK5Atoms.lean` (15, `f2bdc7f`).
Theorem G is kernel-checked at $k = 2$ for every $n gt.eq 2$, at $k = 3$ for
every $n gt.eq 4$, at $k = 4$ for every $n gt.eq 8$, and at $k = 5$ for
every $n gt.eq 14$ — in each case the whole range the statement covers for
that $k$, with no arithmetic gap between the formalised and the written
range. All four discharge
`TverbergStability.StabilityAt k n`, the displayed inequality of Theorem G
with `cVal` equal to $c(n,k)$; the $k = 4$ and $k = 5$ proofs follow this
paper's route step for step, the threshold arithmetic consumed with the
bridge `layer_ratio_lt` checking that $Phi$ rebuilt from the Lean
coefficients is exactly @eq-stab-phi4. The $k = 5$ development proves the
degree-five core expansion @eq-stab-core5 by a peel recursion whose Möbius
weights are derived rather than transcribed, and consumes double
stochasticity through one Cauchy–Schwarz per row (`op_le_of_lines`) with no
operator-norm or Birkhoff import and no square root anywhere. Lemma S1 is kernel-checked at _every_
...
merge and the budget — rests on written proofs plus the exact verifier; that
remains true at every $k$, and no Cheon–Hwang inequality is claimed at
$k = 5$ in any grade. The fixed-dimension certificates of @sec-cells are unformalised, as
...
text; that the hypotheses of `stabilityAt_two`, `stabilityAt_three`,
`stabilityAt_four` and `stabilityAt_five` are the thresholds claimed above;
that `sigma_four_centred` carries both marginal hypotheses; that Lean's
...
`stabilityAt_six` exists — with a non-vacuity check that the same search
does find the four theorems that are there, since an absence found by a
broken search is not an absence. It also runs the orphan diff on the seven
...
28 of 28 declarations in `LayerIdentity.lean`, 67 of 67 declarations in
`StabilityK5Sieve.lean`, 19 of 19 declarations in `StabilityK5.lean` and
16 of 16 declarations in `UniformStability.lean`, and no source contains
`sorry` or `native_decide` outside its comments. All seven counts are parsed
...
that each of the 223 audited declarations reports a subset of `propext`,
...
_What is not claimed, and where the methods stop._ At $k = 4$: nothing
beyond $n gt.eq 10$ and the settled cells $5 lt.eq n lt.eq 9$, and those five
are exact-verifier grade, not Lean. At $k = 5$: nothing beyond
the stability cell $(k = 5, n gt.eq 14)$, now kernel-checked — the
Cheon–Hwang inequality itself is not claimed at
