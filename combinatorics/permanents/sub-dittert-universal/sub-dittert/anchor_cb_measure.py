...
def build_cb(n, out, name=None):
    """
    The 21 `C_b`, with the congruence re-asserted on the built objects.

    `name` selects the stored witness; `None` keeps the historical default
    (`h2_anchor.load_point`'s own).  The design and the constraint SUBSPACE are
    `k`-independent (`NOTES-K5.md` §K5.4, re-measured by
    `diag_core.check_shared_cone`) -- only the witness moves -- so passing a
    `k = n` point here builds that cell's Grams through the same code.  The
    congruence is still RE-ASSERTED below on whatever objects come back, so a
    witness that did not fit the design would fail loudly rather than quietly.
